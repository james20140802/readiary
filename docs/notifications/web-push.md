...
- 설정 UI는 `docs/ui-guidelines.md`의 먹과 종이 토큰, 구분선, 공통 Button·Chip을 사용한다. 시간대 선택 UI는 없으며 설정 저장·기기 등록 시 브라우저가 제공하는 기기 시간대를 자동 저장한다. 앱을 닫아 둔 동안의 시간대 변경은 서버가 감지할 수 없고, 여러 기기를 쓰면 마지막으로 저장한 기기 시간대를 계정 공통 기준으로 사용한다. 미지원 환경·미선택·준비 중·조회 실패 이유를 표시한다.
