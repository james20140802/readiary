...
종류를 모두 합쳐 rolling 7일 최대 2회, 최소 48시간 간격. 같은 시점의 후보를 한 푸시로 묶는다. 설정 저장 시 기기 시간대 기준 20시·모든 요일을 기본값으로 적용한다. 매일 발송한다는 뜻은 아니며 종류별 조건과 전체 빈도 제한을 만족할 때만 발송한다. 내용이 없거나 이미 확인했으면 생략한다. 응답 없는 리마인드 2회 이후에는 쉰다(발송 이력 보관 범위 90일). 완독 알림은 한 번에 최대 5권을 묶고 책별 중복 방지는 해당 책 삭제까지 유지한다.
...
- 설정 UI는 `docs/ui-guidelines.md`의 먹과 종이 토큰, 구분선, 공통 Button을 사용한다. 사용자는 알림 종류만 고르며 시간·요일·시간대 선택 UI는 없다. 시간은 20시, 허용 요일은 모든 요일로 저장하고, 설정 저장·기기 등록 시 브라우저가 제공하는 기기 시간대를 자동 저장한다. 앱을 닫아 둔 동안의 시간대 변경은 서버가 감지할 수 없고, 여러 기기를 쓰면 마지막으로 저장한 기기 시간대를 계정 공통 기준으로 사용한다. 미지원 환경·종류 미선택·준비 중·조회 실패 이유를 표시한다.
...
- 운영 테스트 허용 목록과 만료 시각 등록 완료(본인 계정 1개, 2026-09-15 00:00 KST까지). Preview에는 테스트 허용 목록이나 운영 발송 키를 추가하지 않는다. 코드 머지·운영 배포 후 아이폰에서 확인한다.
