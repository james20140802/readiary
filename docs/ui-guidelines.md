...
- 홈 입력의 실제 값은 16px를 유지하고 안내 placeholder는 15px다. 책 선택 칩은 스크롤하지 않는다. 모바일에서는 선택한 책 한 권(초안 복원 전에는 첫 후보), 640px 이상에서는 최대 세 권을 보여준다. 긴 제목은 말줄임하고 `내 책` 버튼은 항상 남겨 전체 목록에 접근한다.
- 홈의 문장·생각·비공개·남기기는 한 묶음으로 유지한다. 책 칩 옆에 공간이 부족하면 조작부 전체가 다음 줄로 이동하며 내부 버튼은 개별 줄바꿈하지 않는다.
