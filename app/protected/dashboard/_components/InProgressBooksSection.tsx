  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between items-center">
        <span className="text-caption text-ink-faint">진행률</span>
        <span className="text-caption font-bold text-accent">{progress}%</span>
      </div>
    ...
    <Link href={`/protected/books/${book.book_id}`}>
      <Card hoverable variant="flat" className="flex gap-4 items-center p-4">
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between h-[100px] py-1">
          <div>
            <h3 className="text-body font-semibold text-ink line-clamp-2 leading-snug">
              {book.title}
            </h3>
            {book.author && (
  ...
  return (
    <Link href={`/protected/books/${book.book_id}`}>
      <Card hoverable variant="flat" className="flex gap-3 items-center p-3 h-full">
        <div className="relative w-12 h-[68px] shrink-0 rounded-md overflow-hidden">
          <Image
            src={book.cover_url ?? '/images/default-book-cover.png'}
            alt={book.title}
            fill
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between h-[68px] py-0.5">
          <div>
            <h3 className="text-body-sm font-semibold text-ink line-clamp-2 leading-snug">
              {book.title}
            </h3>
            {book.author && (
