 ...
 * `<origin>/auth/confirm?next=…`)를 실어 보낸다. 인증 착지(`/auth/*` — 환경 변수가 옛 클라이언트 착지
 * `/auth/callback`을 가리킬 수도 있다)를 가리키면 오리진과 무관하게 그 안의 `next`(경로)를 꺼낸다 — 메일
 * 링크는 항상 Site URL로 열리므로 로컬·프리뷰에서 요청한 메일도 여기(프로덕션)로 오는데, 목적지 경로만은
 * 살린다. 그 밖의 절대 URL은 같은 오리진일 때만 경로로 풀고, 다른 오리진·루트는 기본 목적지.
 */
