...
-- 알려진 한계: 옛 인스턴스가 같은 기록에 댓글 두 개를 동시에 보내면 두 알림의 연결이 서로 바뀔 수 있다.
-- 알림 행은 댓글 본문을 담지 않고 행위자·기록·종류가 같아 수신자에게 구별되지 않으며, 어느 댓글을 지워도
-- 알림은 정확히 하나 줄어든다. 후보가 여럿일 때 연결을 포기하면 실제 알림을 잃으므로 그쪽을 택하지 않았다.
--
